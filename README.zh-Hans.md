...
文件名，因为安装器会校验嵌入在 wheel 文件名中的版本号。在 macOS 上，终端安装器会把
core wheel 与同版本、匹配当前架构的 `opensquilla-tui-host` companion 成对安装；它不会
安装 Bun，也不会在首次启动时下载 host。
...
`opensquilla_tui_host-0.5.0rc4-py3-none-macosx_11_0_x86_64.whl` 资源。Linux 和
Windows 当前只安装平台无关的 core wheel；它们的 TUI host 将通过各自独立的平台 release
提供：
...
如需在 macOS 上完全锁定版本，请确保 core wheel 和 companion 的 URL 使用同一个 release
