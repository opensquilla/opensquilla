...
routing, SquillaRouter self-learning, safer upgrades with strong profile-data
protection, signed macOS desktop updates, usage and cost reporting, and a
...
session auto-naming that follows the active provider.

### SquillaRouter self-learning

The on-device router now learns from routing outcomes on your own traffic.
The self-learning foundation records decisions, reconciles them with results,
and refines tier choices over time — classification stays fully local, and
the prompt never leaves the machine to make a routing decision. Router
presets for verified coding providers make the initial setup practical.
