"""V020 migration: content-free daily telemetry aggregates."""
...
def test_v020_creates_daily_usage_table(tmp_path: Path) -> None:
    db = str(tmp_path / "v020.db")
    applied = apply_pending(db, MIGRATIONS_DIR)
    assert "V020__telemetry_daily_usage" in applied
...
def test_v020_rollback_drops_daily_usage_table(tmp_path: Path) -> None:
    db = str(tmp_path / "v020-rollback.db")
    apply_pending(db, MIGRATIONS_DIR)
    backend = get_backend(f"sqlite:///{db}")
    migrations = read_migrations(str(MIGRATIONS_DIR))
    by_id = {migration.id: migration for migration in migrations}
    backend.rollback_migrations([by_id["V020__telemetry_daily_usage"]])
