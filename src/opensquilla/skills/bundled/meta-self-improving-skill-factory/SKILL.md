...
description: "Author-by-agent: design a new AgentSkill from a requirement, delegate implementation to coding-agent, then open a PR on GitHub."
kind: meta
meta_priority: 35
always: false
triggers:
...
Agent-authored skills: design → implement (delegated to `coding-agent`)
...
coding-agent (or shell tooling) to fill in scripts, then open a PR.
