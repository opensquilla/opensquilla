...
文件名，因为安装器会校验嵌入在 wheel 文件名中的版本号。在 macOS 和 Linux 上，终端安装器会把
core wheel 与同版本、匹配当前架构的 `opensquilla-tui-host` companion 成对安装；它不会
要求 Bun 或源码检出，也不会在首次启动时下载 host。
...
在 macOS 上，这条命令会从同一个 release 成对安装 core wheel 和匹配架构的 TUI host。
Apple Silicon 对应的完全锁定版本命令如下：
...
Intel Mac 使用同一 release 中的
`opensquilla_tui_host-0.5.0rc4-py3-none-macosx_11_0_x86_64.whl` 资源。Linux 根据
架构使用 `opensquilla_tui_host-0.5.0rc4-py3-none-manylinux_2_28_x86_64.whl`
或 `opensquilla_tui_host-0.5.0rc4-py3-none-manylinux_2_28_aarch64.whl`。原生
Windows TUI host 将通过后续独立平台 release 提供。

安装器会把 core 和 companion 放进同一个隔离的工具环境，再由 `uv` 下载所选 extra 所声明的依赖。
...
在这个 macOS 和 Linux RC 中，`opensquilla chat --ui tui` 会启动全屏 TUI。裸命令
`opensquilla chat` 暂时仍使用最小化的 `plain` 救援 renderer；`--ui auto` 优先选择
TUI，但只允许在进入全屏前回退到 `plain`。安装、升级、重装和回滚会让 core 与
companion 始终保持同一版本。
