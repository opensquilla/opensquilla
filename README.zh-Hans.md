...
每个入口——Web UI、CLI 和聊天渠道——都跑在同一个循环里，因此工具调度、重试和决策日志的行为处处一致。可插拔的提供商层对接 OpenRouter、OpenAI、Anthropic、
Ollama、DeepSeek、Gemini、Qwen/DashScope 等 20 多个 LLM 提供商，无需改动你的代码或
配置结构。

OpenSquilla 0.5.0 Preview 2 是当前预览发布版本。
...
对于 0.5.0 Preview 2 的桌面使用，建议从 GitHub Release 下载打包桌面安装包:macOS 上为
`OpenSquilla-0.5.0-rc2-mac-arm64.dmg`，Windows 上为 `OpenSquilla-0.5.0-rc2-win-x64.exe`。
...
0.5.0 Preview 2 桌面安装包将 Vue 控制台和网关运行时打包在一个 Electron 外壳中。
...
| **20+ 个 LLM 提供商** | 提供商注册表面向 20 多个 LLM 后端——OpenRouter、OpenAI、Anthropic、Ollama、DeepSeek、Gemini、DashScope/Qwen、Moonshot、Mistral、Groq、Zhipu、SiliconFlow、vLLM、LM Studio 等等，并支持主用 + 回退选择；首次 onboarding 会暴露已验证的子集。 |
