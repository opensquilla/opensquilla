...
test.describe('Tool rows and activity ribbon', () => {
  ...
  test('live search run shows the lightweight activity process, then collapses', async ({ page }) => {
    ...
    // The live run keeps phase narration and a right-aligned step chip without
    // wrapping the process in card chrome.
    ...
    const observed = await page.evaluate(async () => {
      const t0 = Date.now()
      const samples: Array<{ phase: string | null; step: string | null; elapsed: string | null; activityRows: number }> = []
      while (Date.now() - t0 < 180000) {
        ...
        const activityRows = document.querySelectorAll('.work-card .tool-timeline--activity .tool-row').length
        samples.push({
    ...
    // Activity rows render while the live turn owns the focus.
    expect(observed.some((s) => s.activityRows > 0)).toBe(true)
  ...
  test('live failed tool calls stay out of the visible activity process', async ({ page }) => {
    test.skip(!LIVE, 'Live gateway test; set OPENSQUILLA_E2E_LIVE=1 to run.')
    test.setTimeout(240000)
    ...
    await expect(workCard).toBeVisible({ timeout: 30000 })
    await expect(workCard).toHaveCount(0, { timeout: 180000 })
    await expect(page.locator('.tool-row--error')).toHaveCount(0)
    await expect(page.locator('.tool-row-section--error')).toHaveCount(0)
  })
})
