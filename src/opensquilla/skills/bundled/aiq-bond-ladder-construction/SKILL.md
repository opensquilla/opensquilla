...
- Query each maturity year with `securities_search(credit_grade='IG', maturity_start=<year-01-01>,
  maturity_end=<year-12-31>, include_liquidity='true', liquidity_lookback_days='30',
  order_by='trades')`. Select one real row per year and preserve every explicit range/rung the user
  supplied.
...
- Show at least Rung Year, Security, CUSIP, Maturity, Rating, Yield, 30D Trades, and Notional.
- Never say "no portfolio was created" and stop; produce the tool-backed rungs.

## Full portfolios
"Build a $100mm IG portfolio mirroring LQD, outperform by 25bps, 35 holdings" → map every stated constraint to `generate_portfolio_proposal` parameters (size, benchmark=LQD/IG, target yield pickup, num_holdings) and return the proposal table (metric | target | actual | status) + sector allocation.
