...
def _complete_pending_record(
    ...
    *,
    ...
    try:
        ...
        if ensemble_enabled:
            # Feedback attribution is deliberately stricter than the
            # observability fallback above: only the configured identity of
            # the final request can name the model whose text the user read.
            # If that trace is absent, fail closed instead of crediting the
            # classifier's requested model.
            record["model"] = _final_request_model(ensemble_trace)
        elif executed_model is not None:
