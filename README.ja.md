...
リリース版のインストールコマンドは、公開された GitHub リリースのアセットを使います。Python wheel のインストールでは、バージョン付きの wheel ファイル名を使います。インストーラーが wheel ファイル名に埋め込まれたバージョンを検証するためです。macOS では、ターミナルインストーラーがこの core wheel と、同じバージョンかつアーキテクチャ固有の `opensquilla-tui-host` companion を組み合わせます。Bun をインストールしたり、初回起動時に host をダウンロードしたりすることはありません。
...
Intel Mac では、同じリリースの `opensquilla_tui_host-0.5.0rc4-py3-none-macosx_11_0_x86_64.whl` アセットを使います。Linux と Windows は現在、プラットフォーム非依存の core wheel のみをインストールします。TUI host アセットは、それぞれ別のプラットフォームリリースで追加予定です:
...
macOS で完全にバージョンを固定する場合は、core wheel と companion の URL を必ず同じリリースタグにしてください。リリースインストーラーはこれを自動で行い、互換性のないバージョンを拒否します。
