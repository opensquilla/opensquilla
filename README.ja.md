...
リリース版のインストールコマンドは、公開された GitHub リリースのアセットを使います。Python wheel のインストールでは、バージョン付きの wheel ファイル名を使います。インストーラーが wheel ファイル名に埋め込まれたバージョンを検証するためです。macOS と Linux では、ターミナルインストーラーがこの core wheel と、同じバージョンかつアーキテクチャ固有の `opensquilla-tui-host` companion を組み合わせます。Bun、ソースのチェックアウト、初回起動時の host ダウンロードは不要です。
...
**2. OpenSquilla をインストールする。**

macOS と Linux では、リリースインストーラーがプラットフォームに合うアセットを選び、`uv tool install` を実行します:

```sh
curl -LsSf https://opensquilla.ai/install.sh | bash
```

macOS では、このコマンドが同じリリースの core wheel とアーキテクチャ固有の TUI host を一緒にインストールします。Apple Silicon 向けの、同等で完全にバージョン固定されたコマンドは次のとおりです:
...
Intel Mac では、同じリリースの `opensquilla_tui_host-0.5.0rc4-py3-none-macosx_11_0_x86_64.whl` アセットを使います。Linux はアーキテクチャに応じて `opensquilla_tui_host-0.5.0rc4-py3-none-manylinux_2_28_x86_64.whl` または `opensquilla_tui_host-0.5.0rc4-py3-none-manylinux_2_28_aarch64.whl` を使います。ネイティブ Windows の TUI host は別のプラットフォームリリースで提供します。

インストーラーは core と companion を同じ隔離された tool 環境に配置し、続いて `uv` が、選択した extra が宣言する依存関係をダウンロードします。デフォルトの `recommended` extra には、ONNX Runtime、LightGBM、NumPy、tokenizers といった SquillaRouter のランタイム依存関係が含まれるため、これらの wheel がすでにキャッシュされていない限り、初回インストールにはネットワークアクセスが必要です。`uv` は macOS の `libomp` や Windows の Visual C++ Redistributable のようなシステムネイティブのランタイムはインストールしません。ルーターランタイムがネイティブライブラリの読み込みエラーを報告した場合は、[トラブルシューティング](#troubleshooting)を参照してください。
...
この macOS / Linux RC では、`opensquilla chat --ui tui` でフルスクリーン TUI を起動します。オプションなしの `opensquilla chat` は最小限の `plain` レンダラーのままです。`--ui auto` は TUI を優先し、フルスクリーン開始前に限り `plain` へフォールバックできます。インストール、アップグレード、再インストール、ロールバックでは core と companion を同じバージョンに保ちます。

> [!NOTE]
> 新規の `uv` インストール直後に `opensquilla` が見つからない場合は、新しいターミナルを開くか、ステップ 1 の PATH 設定の行を再実行してください。

macOS または Linux で完全にバージョンを固定する場合は、core wheel と companion の URL を必ず同じリリースタグにしてください。リリースインストーラーはこれを自動で行い、互換性のないバージョンを拒否します。
