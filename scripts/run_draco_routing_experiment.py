...
def select_tasks_by_ids(
    ...
    duplicates = sorted(
        task_id for task_id in set(normalized) if normalized.count(task_id) > 1
    )
    if duplicates:
    ...
    if repeated_source_ids:
        raise ValueError(
            "selected task id(s) are not unique in the input: "
            + ", ".join(repeated_source_ids)
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Failed rows remain invalid and the process still exits with status 2."
        ),
    )
    parser.add_argument(
