...
V022_ID = "V022__telemetry_daily_usage"
...
def test_v022_rollback_drops_daily_usage_table(tmp_path: Path) -> None:
    ...
    backend.rollback_migrations([by_id[V022_ID]])
...
def test_v022_dependency_matches_the_rc4_migration_tip() -> None:
    migrations = read_migrations(str(MIGRATIONS_DIR))
    by_id = {migration.id: migration for migration in migrations}

    assert {migration.id for migration in by_id[V022_ID].depends} == {
        "V019__turn_errors"
    }
