"""Opt-in live agent E2E gate for the normalized research_search tool.
...
async def test_live_agent_uses_research_search_without_web_fetch_loop() -> None:
    ...
    async def tool_handler(call: ToolCall) -> ToolResult:
        ...
        if call.tool_name == "research_search":
        ...
        if call.tool_name == "web_search":
            web_search_queries.append(str(call.arguments.get("query") or ""))
    ...
    agent = Agent(
        ...
        tool_definitions=[
            _tool_def(
                "research_search",
                "Run one normalized research search and return compact cited results.",
                {"query": {"type": "string"}},
            ),
            _tool_def(
                "web_search",
