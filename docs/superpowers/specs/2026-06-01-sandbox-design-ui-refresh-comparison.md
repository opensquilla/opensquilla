...
另外，开发流程如果只跑局部测试，就可能出现“沙箱功能自己绿了，但项目其他测试被破坏”的情况。`/home/lrk/opensquilla/tests` 是全项目测试目录，当前基线应该先是绿的。

**优化后设计**

开发门禁改成：

1. 开发前先在仓库根目录跑全量 `pytest tests`。
2. 如果开发前全量测试不通过，先停止沙箱开发；如果已经有自己的预备改动，就只撤回自己的改动，然后单独处理基线失败。
3. 开发中可以跑局部测试加快反馈。
4. 开发后必须再次跑全量 `pytest tests`。
5. 如果开发后全量测试不通过，就修改实现代码直到全量通过。
6. 不通过时不要为了修绿去改其他领域测试；非沙箱测试不是这次沙箱改动的修改面。
7. 等现有全量测试通过后，再追加新的沙箱功能测试。新增沙箱测试失败时，也应该修实现代码，而不是改无关测试。

原有测试不改、不删、不放松断言。沙箱新能力的测试只新增覆盖，风格跟附近测试保持一致：
