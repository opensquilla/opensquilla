...
release tag, for example `v0.5.0rc3`.

## Failure model

The mirror workflow fails if required OSS configuration is missing, if the
GitHub Release has no downloadable assets, if `SHA256SUMS` is missing, or if the
checksum verification fails before upload. In those cases, GitHub remains the
source of truth and OSS should not be treated as a complete mirror for that tag.
