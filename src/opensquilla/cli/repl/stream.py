...
class WaitingIndicator:
    ...
    # kept in sync with chat.js SQUILLA_VERBS — calmer sensory register so
...
class StreamingRenderer:
    ...
    async def _awrite_payload(self, payload: str) -> None:
        ...
        if self._stream_write is not None:
            ...
            return
        stream_output = getattr(self._chat_app, "stream_output", None)
    ...
    def _open_squilla_status_line(self) -> str:
    ...
    def _begin_stream(self) -> str:
        ...
        if self._stream_started:
            return ""
        self._stop_waiting()
        marker = self._open_squilla_status_line()
    ...
    def append_text(self, delta: str) -> None:
        ...
        if not visible:
            ...
            # an empty ``◢ squilla  `` doesn't print for a directive-only
            # turn.
            return
    ...
    async def aappend_text(self, delta: str) -> None:
        ...
        ``ChatApplication.write_through`` which holds the output lock for
        the microsecond write window. When no `chat_app` was attached the
